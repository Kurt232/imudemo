...
def extract_sensor(data, time_index, time_tag, window_time):
  index = time_index
  while index < len(data) and abs(data.iloc[index]['Creation_Time'] -
  ...
  if index == time_index:
    ...
    if data_slice['User'].unique().size > 1 or data_slice['gt'].unique(
    ).size > 1:
      return None, index
    else:
...
def preprocess_hhar(path,
                    ...
                    seq_len=40,
                    jump=0):
  accs = pd.read_csv(os.path.join(path, 'Phones_accelerometer.csv'))
  gyros = pd.read_csv(os.path.join(path, 'Phones_gyroscope.csv'))  #, nrows=200000
  ...
  while time_index[0] < len(accs) and time_index[1] < len(gyros):
    acc, time_index_new_acc = extract_sensor(accs,
                                             time_index[0],
                                             time_tag,
                                             window_time=window_time *
                                             pow(10, 6))
    gyro, time_index_new_gyro = extract_sensor(gyros,
                                               time_index[1],
                                               time_tag,
                                               window_time=window_time *
                                               pow(10, 6))
    ...
    if acc is not None and gyro is not None and np.all(acc[-3:] == gyro[-3:]):
      ...
      if time_index[0] < len(accs) and time_index[1] < len(gyros):
        time_tag = min(accs.iloc[time_index[0], 2], gyros.iloc[time_index[1],
                                                               2])
      else:
        break
  data_raw = np.array(data)
  data_new, label_new = separate_data_label(data_raw)
  np.save(os.path.join(path_save, 'data_' + version + '.npy'),
          np.array(data_new))
  np.save(os.path.join(path_save, 'label_' + version + '.npy'),
          np.array(label_new))
